...
class mywindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def init_ser(self):
        if self.ser is None:
            ...
            self.ser = serial.Serial(port_list[1].device, 9600, timeout=0.5)
    ...
    # 串口控制
    def out_serial(self):
        if self.predict_index == 2 or not self.ser:
            return
    ...
    def work_thread(self, cam=0, pData=0, nDataSize=0):
        ...
        while True:
            ...
            if ret == 0:
                ...
                image_height, image_width, image_depth = image.shape  # 读取图像高宽深度 输入为rgb格式
                self.image_show = QImage(image.data, image_width, image_height, image_width * image_depth,
                                         QImage.Format_RGB888)

                # image_height, image_width = image.shape  # 读取图像高宽 输入为gray格式
